...
// 处理礼品卡状态更新
const handleGiftCardStatusUpdate = (data) => {
  try {
    console.log('🎁 处理礼品卡状态更新:', data)

    const { gift_card_number, status, message } = data

    if (!gift_card_number || !status) {
      console.warn('⚠️ 无效的礼品卡状态更新数据')
      return
    }

    // 更新本地礼品卡列表中的状态
    const giftCard = giftCards.value.find(card => card.gift_card_number === gift_card_number)
    if (giftCard) {
      giftCard.status = status
      console.log(`📝 已更新本地礼品卡状态: ${gift_card_number.substring(0, 4)}**** -> ${status}`)
    }

    // 显示状态更新通知
    const notificationType = status === '有额度' ? 'success' : 'warning'
    ElNotification({
      title: '礼品卡状态更新',
      message: `${gift_card_number.substring(0, 4)}**** - ${message}`,
      type: notificationType,
      duration: 5000
    })

    // 如果当前在礼品卡管理页面，刷新列表
    if (activeTab.value === 'gift-cards') {
      loadGiftCards()
    }

    console.log(`✅ 礼品卡状态更新已处理: ${gift_card_number.substring(0, 4)}**** -> ${status}`)
  } catch (error) {
    console.error('处理礼品卡状态更新错误:', error)
  }
}
